...
def graficar_con_kmeans_optimizado(ax, data, x_column, y_column, n_clusters, max_iter=300, n_init=10, cluster_seleccionado=None):
    ...
    if cluster_seleccionado is not None:  # Filtrar y graficar solo el cluster seleccionado
        clustered_data = data[data['Cluster'] == cluster_seleccionado]
        ax.scatter(clustered_data[x_column], clustered_data[y_column], color=colors(cluster_seleccionado), label=f'Cluster {cluster_seleccionado}', alpha=0.6)
    else:  # Graficar todos los clusters
...
def seleccionar_columnas_clusters_iteraciones(file_path):
    ...
    # Variable para filtrar los clusters
    filtrar_cluster = BooleanVar()
    filtrar_cluster.set(False)

    # Checkbox para filtrar el cluster
    ...
    def actualizar_graficas(*args):
        ...
        graficar_con_kmeans_optimizado(ax1, data, x_column.get(), y_column.get(), n_clusters, max_iter=max_iter, n_init=10, cluster_seleccionado=cluster_seleccionado)
